...
function markAvailable(key){
  state.statusMap.set(key,'Available');
  addToList('list-available',key,'available');
  removeFromList('list-used',key);
  renderCounters();

  const s = document.getElementById('search');
  if (s) {
    const ua = navigator.userAgent || '';
    const isMobile = /iP(hone|od|ad)|Android|Mobi/i.test(ua);
    if (!isMobile) { s.focus(); s.select(); }
    else { s.blur(); }
  }
...
function finishSession(){
  ...
  // iOS/WebKit blocks multiple automatic downloads from a single tap.
  // Workaround: one file per tap (tap Finish twice).
  if (isIOSWebKit) {
    if (!state.pendingUsedExport) {
      exportCSV(available, headers, `${state.sessionId}-available`);
      state.pendingUsedExport = { rows: used, headers, name: `${state.sessionId}-used` };
      toast('Available CSV created. Tap Finish again to download USED.');
      return;
    }
    exportCSV(state.pendingUsedExport.rows, state.pendingUsedExport.headers, state.pendingUsedExport.name);
    state.pendingUsedExport = null;
    toast('Used CSV created');
    return;
  }

  // Desktop browsers: dual export in one click
  exportCSV(available, headers, `${state.sessionId}-available`);
  setTimeout(() => {
