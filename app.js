...
async function init(){
  ...
  if(cancelBtn){
    ...
    const closeManualModal = (e)=>{
      ...
      try{ frm.reset(); }catch(_){}
      // iOS: let the keyboard dismiss before closing the dialog
      setTimeout(()=>{ try{ dlg.close(); }catch(_){ try{ dlg.removeAttribute('open'); }catch(__){} } }, 0);
    };
    cancelBtn.addEventListener('click', closeManualModal);
    cancelBtn.addEventListener('touchend', closeManualModal, { passive:false });
  }
  if(dlg){
