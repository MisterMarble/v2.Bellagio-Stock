...
function markAvailable(key){ state.statusMap.set(key,'Available'); addToList('list-available',key,'available'); removeFromList('list-used',key); renderCounters(); const s=document.getElementById('search'); if(!isMobileDevice()){ s.focus(); s.select(); } refreshResultsIfVisible(); }
...
function finishSession(){
  ...
  // iOS/WebKit blocks two downloads from a single tap. On iOS: export one file per tap.
  ...
  if (isIOS){
    if (!state.pendingUsedExport){
      ...
      state.pendingUsedPayload = { used, headers, name: `${state.sessionId}-used` };
      toast('Available exported. Tap Finish again for USED.');
      return;
    }
    // Second tap: USED
    const payload = state.pendingUsedPayload || { used, headers, name: `${state.sessionId}-used` };
    exportCSV(payload.used, payload.headers, payload.name);
    state.pendingUsedExport = false;
    state.pendingUsedPayload = null;
    toast('Used exported.');
    return;
  }

  // Desktop/Android: export both (slight delay helps some browsers)
  exportCSV(available, headers, `${state.sessionId}-available`);
...
\1
  state.pendingUsedExport = false;
  state.pendingUsedPayload = null; const raw = localStorage.getItem('bellagio_session'); if(!raw){ toast('No saved session'); return; } const dump = JSON.parse(raw); const manuals = dump.manual || []; for(const m of manuals){ if(!state.slabs.find(s=>s._rowid===m._rowid)){ state.slabs.push(m); } } buildIndex(state.slabs); state.statusMap = new Map(dump.status); renderCounters(); toast('Session resumed'); }
\1
  state.pendingUsedExport = false;
  state.pendingUsedPayload = null;
  // New ID & show it
