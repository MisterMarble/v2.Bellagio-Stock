...
function setupMaterialAutocomplete(){
  ...
  if(!input) return;
  let dl = document.getElementById('materialDatalist');
  if(!dl){
    dl = document.createElement('datalist');
    dl.id = 'materialDatalist';
    document.body.appendChild(dl);
  }
  if(input.getAttribute('list') !== 'materialDatalist'){
    input.setAttribute('list','materialDatalist');
    // keep original behaviour: do not force change events etc.
  }
...
async function init(){
  ...
  // Manual input: batch number keypad (numeric)
  const manualBatchNumber = document.querySelector('#addSlabForm input[name="batch_number"]');
  if(manualBatchNumber){
    manualBatchNumber.setAttribute('inputmode','numeric');
    manualBatchNumber.setAttribute('pattern','[0-9]*');
    // iOS-friendly numeric keypad hint
    manualBatchNumber.setAttribute('type','tel');
  }
  ...
  const cancelBtn = document.getElementById('cancelAdd') || dlg?.querySelector('button[value="cancel"]');
  if(cancelBtn && dlg && frm){
    cancelBtn.type = 'button';
    cancelBtn.addEventListener('click', (e)=>{
      e.preventDefault();
      // 1st click: if a field is focused, just dismiss the keypad
      const active = document.activeElement;
      if(active && dlg.contains(active) && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA')){
        active.blur();
        return;
      }
      // 2nd click: close modal
      frm.reset();
      dlg.close();
    });
  }
