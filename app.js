...
// ---- end helper ----
function markAvailable(key){ state.statusMap.set(key,'Available'); addToList('list-available',key,'available'); removeFromList('list-used',key); renderCounters(); const s=document.getElementById('search'); const isMobile=/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent||''); if(isMobile){ try{s.blur();}catch(e){} } else { s.focus(); s.select(); } refreshResultsIfVisible(); }
...
function finishSession(){
  ...
  // iOS/WebKit blocks two downloads from one tap.
