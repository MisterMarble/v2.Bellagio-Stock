...
const state = {
  ...
  rowIndex: [],
  deferredPrompt: null,
  pendingUsedExport: false
};


const STORAGE_KEY = 'bellagio_status';
...
function persistState(){
  const payload = {
    ...
    pendingUsedExport: !!state.pendingUsedExport,
    status: Array.from(state.statusMap.entries())
  };
...
function restoreState(){
  ...
  try{
    ...
    if(payload && Array.isArray(payload.status)){
      state.statusMap = new Map(payload.status);
    }
    if(payload && typeof payload.pendingUsedExport === 'boolean'){
      state.pendingUsedExport = payload.pendingUsedExport;
    }else{
      state.pendingUsedExport = false;
    }
  }catch(e){}
}
...
// ---- end helper ----
function markAvailable(key){ state.statusMap.set(key,'Available'); addToList('list-available',key,'available'); removeFromList('list-used',key); renderCounters();   const s=document.getElementById('search');
  const ua = navigator.userAgent || '';
  const isMobile = /iPhone|iPad|iPod|Android|Mobi/i.test(ua) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
  if(isMobile){
    s.blur();
  }else{
    s.focus();
...
function markUsed(key){ state.statusMap.set(key,'Used'); addToList('list-used',key,'used'); removeFromList('list-available',key); renderCounters(); persistState();
  refreshResultsIfVisible(); }
...
function openAddModal(){ document.getElementById('addSlabModal').showModal(); setupMaterialAutocomplete(); document.querySelector('#addSlabForm input[name=slab_number]').focus(); }


function setupMaterialAutocomplete(){
  const input = document.querySelector('#addSlabForm input[name=material_name]');
  if(!input) return;

  // Build unique materials from current inventory (if any)
  const set = new Set();
  if(typeof state !== 'undefined' && state && Array.isArray(state.rowIndex)){
    for(const r of state.rowIndex){
      const name = (r && r.material_name ? String(r.material_name) : '').trim();
      if(name) set.add(name);
    }
  }
  const materials = Array.from(set).sort((a,b)=>a.localeCompare(b, undefined, {sensitivity:'base'}));

  // Create (once) a lightweight suggestion box (no CSS file changes).
  let box = document.getElementById('materialSuggestBox');
  if(!box){
    box = document.createElement('div');
    box.id = 'materialSuggestBox';
    ...
    // Attach inside the dialog so it stays above the modal.
    const dlg = document.getElementById('addSlabModal');
    (dlg || document.body).appendChild(box);
  }

  function positionBox(){
    const r = input.getBoundingClientRect();
    const dlg = document.getElementById('addSlabModal');
    const dlgRect = dlg ? dlg.getBoundingClientRect() : {left:0, top:0};

    const insideDlg = dlg && box.parentElement === dlg;
    const left = insideDlg ? (r.left - dlgRect.left) : r.left;
    const top  = insideDlg ? (r.bottom - dlgRect.top + 6) : (r.bottom + 6);
  ...
  function showMatches(){
    ...
    if(!q || materials.length===0){ hideBox(); return; }
    ...
    for(const name of matches){
      ...
      // Use pointerdown to avoid blur/tap timing issues on mobile.
  ...
  // Wire once per input (idempotent)
  if(!input.dataset.materialAutocompleteWired){
    input.dataset.materialAutocompleteWired = '1';
...
function finishSession(){
  ...
  for(const r of state.rowIndex){
    const status = state.statusMap.get(r.key);
    if(r.source==='master' && (status==='Used' || status==='Untouched')){
      used.push({combined_id:r.combined_id, material_name:r.material_name, size_mm:r.size_mm, thickness_mm:r.thickness_mm, status:'Used', source:r.source});
    }
    if(status==='Available'){
      available.push({combined_id:r.combined_id, material_name:r.material_name, size_mm:r.size_mm, thickness_mm:r.thickness_mm, status:'Available', source:r.source});
    }
  }
  const headers=['combined_id','material_name','size_mm','thickness_mm','status','source'];

  // Persist before any navigation/OS download UI can background the page.
  persistState();

  // iOS/WebKit allows one download per user gesture.
  const ua = navigator.userAgent || '';
  const isIOS = /iPhone|iPad|iPod/i.test(ua) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);

  if(isIOS){
    if(!state.pendingUsedExport){
      exportCSV(available, headers, `${state.sessionId}-available`);
      state.pendingUsedExport = true;
      persistState();
      toast('CSV files created');
      return;
    }else{
      exportCSV(used, headers, `${state.sessionId}-used`);
      state.pendingUsedExport = false;
      persistState();
      toast('CSV files created');
      return;
  ...
  exportCSV(available, headers, `${state.sessionId}-available`);
  setTimeout(()=>exportCSV(used, headers, `${state.sessionId}-used`), 250);
  toast('CSV files created');
...
async function init(){
  ...
  // mobile keyboard hint: numeric-only (Option A)
  ...
  // Manual input: slab number keypad (numeric)
  const manualSlabNumber = document.querySelector('#addSlabForm input[name="slab_number"]');
  if(manualSlabNumber){
    manualSlabNumber.setAttribute('inputmode','numeric');
    manualSlabNumber.setAttribute('pattern','[0-9]*');
  }
