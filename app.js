...
async function init(){
  ...
  document.getElementById('addSlabForm').addEventListener('submit', addManualSlab); (function(){const rm=()=>document.querySelectorAll('#list-used button[data-action="remove"],#list-available button[data-action="remove"]').forEach(b=>b.remove());rm();const U=document.getElementById('list-used'),A=document.getElementById('list-available');if(U)new MutationObserver(()=>rm()).observe(U,{childList:true,subtree:true});if(A)new MutationObserver(()=>rm()).observe(A,{childList:true,subtree:true});})();

  // ---- Manual modal Cancel: robust on iOS (keyboard open) ----
  {
    ...
    const closeManualModal = (ev) => {
      try { ev?.preventDefault?.(); ev?.stopPropagation?.(); } catch(e) {}
      // If an input is focused, iOS may treat first tap as "dismiss keyboard".
      // Blur first, then close on next tick so the same gesture still closes the modal.
      try { document.activeElement?.blur?.(); } catch(e) {}
      setTimeout(() => {
        try { frm?.reset?.(); } catch(e) {}
    ...
    if (cancelBtn) {
      ...
      // pointerup works on modern mobile + desktop; click is fallback.
      cancelBtn.addEventListener('pointerup', closeManualModal);
      cancelBtn.addEventListener('click', closeManualModal);
    }
    // Optional: clicking backdrop closes as before.
    if (dlg) {
      dlg.addEventListener('mousedown', (e) => {
        if (e.target === dlg) closeManualModal(e);
      });
    }
  }
  // ---- end manual cancel fix ----
